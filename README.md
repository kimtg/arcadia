...
## Library
`++ -- <= = >= abs acons afn alist and append atom avg best caar cadr carif cddr copy def do do1 each even firstn for idfn iflet in isa isnt join len let list map map1 max med median merge mergesort min multiple nearest no nthcdr number odd or pair positive quasiquote reduce reverse rfn round roundup rreduce sort sref uniq unless w/uniq when whenlet with withs`
