...
## Library
`++ -- <= = > >= abs afn and append avg best caar cadr carif cddr def do do1 each even firstn for idfn iflet isa isnt join len let list map map1 max min multiple nearest no nthcdr number odd or pair positive quasiquote reduce reverse rfn round roundup rreduce sref uniq unless w/uniq when whenlet with withs`
